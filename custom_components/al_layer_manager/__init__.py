...
async def _ensure_services_registered(hass: "HomeAssistant") -> None:
    ...
    async def handle_set_mode(call) -> None:
        ...
        zone_id = payload["zone_id"]
        runtime = await _resolve_runtime(zone_id)
        stored = runtime.engine.zones[zone_id].modes.get(payload["mode"])
        transition = int(
            payload.get(
                "transition_seconds",
                stored.transition_seconds if stored else 2,
            )
        )
        profile = ModeProfile(
            brightness_multiplier=float(payload["brightness_multiplier"]),
            kelvin_adjustment=int(payload["kelvin_adjustment"]),
            transition_seconds=transition,
        )
        runtime.engine.zones[zone_id].modes[payload["mode"]] = profile
        await runtime.async_set_mode(zone_id, payload["mode"], profile)

    async def handle_clear_mode(call) -> None:
        payload = dict(call.data)
        zone_id = payload["zone_id"]
        runtime = await _resolve_runtime(zone_id)
        await runtime.async_clear_mode(zone_id, payload["mode"])
