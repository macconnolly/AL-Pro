...
class LayerManagerRuntime:
    ...
    async def async_set_mode(self, mode: str, profile: ModeProfile, zone_id: Optional[str] = None) -> None:
        computation = self.engine.apply_mode(mode, profile)
        self._publish_mode_telemetry(zone_id or "*", computation)

    async def async_clear_mode(self, mode: str, zone_id: Optional[str] = None) -> None:
        self.engine.clear_mode(mode)
        self._publish_mode_telemetry(zone_id or "*", None)
